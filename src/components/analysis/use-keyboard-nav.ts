...
interface KeyboardNavOptions {
  total: number;
  /** Jump to an absolute ply (Home/End); always on the mainline. */
  onSeek: (ply: number) => void;
  /** Step one half-move back — variation-aware. */
  onStepBack: () => void;
  /** Step one half-move forward — variation-aware. */
  onStepForward: () => void;
  onFlip: () => void;
}

/** Global board navigation: →/Space step forward, ← step back, ↑/Home jump to
 * start, ↓/End to the end, F flips the board. Typing in
 * an input is ignored.
...
export function useKeyboardNav(options: KeyboardNavOptions) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      const { total, onSeek, onStepBack, onStepForward, onFlip } = ref.current;
      if (e.key === "ArrowRight" || e.key === " ") onStepForward();
      else if (e.key === "ArrowLeft") onStepBack();
