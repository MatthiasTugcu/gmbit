/* Gmbit — game data: Morphy vs. Duke of Brunswick & Count Isouard, Paris 1858 ("The Opera Game")
   Public domain. Evals + classifications crafted for the analyzer demo. */
(function () {
  // classification keys: brilliant, best, good, book, inaccuracy, mistake, blunder
  // cp = centipawns from White's POV after the move. mate = {mate:n} (n plies-to-mate, sign = side).
  const moves = [
  ...
  const players = {
    ...
    event: 'Paris Opera House, 1858', result: '1–0', opening: 'Philidor Defence',
  };

  const engine = { name: 'Gmbit Engine 16', kind: 'NNUE', depth: 24 };

  window.GMBIT = { moves, players, engine };
})();
